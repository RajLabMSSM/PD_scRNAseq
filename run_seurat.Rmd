...
appendedGenes <- c(pbmc@var.genes, "CD14", "FCGR3A")
pbmc <- ScaleData(object = pbmc, genes.use = appendedGenes , vars.to.regress = c("nUMI", "percent.mito"), 
...
PCHeatmap(object = pbmc, pc.use = 1:12,do.balanced=T, label.columns=F, use.full=F)   # cells.use = 500, 
```
...
```{r UMAP, results = 'asis'} 
# cat(print(mem_used()))
pbmc <- RunUMAP(object = pbmc, reduction.use = "pca", dims.use = 1:10, verbose=TRUE, num_threads=nCores) # , num_threads=0
# cat(print(mem_used()))
# Plot results
...
pbmc <- RunTSNE(object=pbmc,  reduction.use = "pca", dims.use = 1:10, do.fast = TRUE,
                tsne.method = "Rtsne", num_threads=nCores, verbose=T) #   FItSNE
# cat(print(mem_used()))
...
Default fig.height and fig.width is 7
```{r tSNE dx, out.width="150%", out.height="150%"}
tSNE_metadata_plot("dx") 
```

### Mutations

```{r tSNE mut, out.width="150%", out.height="150%"}
tSNE_metadata_plot("mut") 
```

### Gender

```{r tSNE Gender, out.width="150%", out.height="150%"}
tSNE_metadata_plot("Gender") 
```

### Age

```{r tSNE Age, out.width="150%", out.height="150%"}
tSNE_metadata_plot("Age") 
```
### Individual ID

```{r tSNE Individual ID, out.width="150%", out.height="150%"}
...
```{r Identify Cell Types By Average Expression, out.width="150%", out.height="150%"}  
...
runDGE <- function(pbmc, meta_var, group1, group2, test.use="wilcox"){
  ...
  DEGs <- FindMarkers(pbmc, ident.1=group1, ident.2=group2, test.use=test.use, only.pos = F)
...
```{r All Cells: CellType_AvgExp}
DEG_df <-runDGE(pbmc, "CellType_AvgExp", "CD14++/CD16+", "CD14++/CD16--") 
```
...
```{r Within Clusters: Between Disease Groups}   
DGE_within_clusters(pbmc, "dx", "PD", "control")    
```

## Between Mutation Groups {.tabset .tabset-fade .tabset-pills}

```{r Within Clusters: Between Mutation Groups}
DGE_within_clusters(pbmc, "mut", "LRRK2", "PD")
```

## Between Mutation Groups {.tabset .tabset-fade .tabset-pills}

```{r Within Clusters: CellType_DGE}
DGE_within_clusters(pbmc, "CellType_DGE", "CD14++/CD16+", "CD14++/CD16--") 
```

```{r Within Clusters: CellType_AvgExp}
...
plot2 <- TSNEPlot(object = pbmc, do.return = TRUE, group.by = "post_clustering", 
...
FeaturePlot(object = pbmc, features.plot = top1_res3.0$gene, cols.use = c("green", "blue"))
