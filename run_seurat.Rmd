...
for (clust in unique(DAT.markers$cluster)){
   ...
   createDT(DEG_df, caption = cap) %>% print()
   cat('\n')   
} 
```
...
## Between CellType_AvgExp {.tabset .tabset-fade .tabset-pills}

```{r Within Clusters: CellType_AvgExp, results='asis'}
...
for (clust in unique(DAT.markers.sig$cluster)){
  ...
  for (db in enrichr_dbs){
    cat('\n')
    cat("#### ",db,"\n")  
    createDT(subset(results[[db]], Adjusted.P.value<=0.05), paste("Enrichr Results: ",db,"Cluster ", clust)) %>% print()
    cat('\n')
  } 
  cat('\n')
} 
...
for (mod in unique(modules$moduleColors)){
  ...
  for (db in enrichr_dbs){
    cat('\n')
    cat("#### ",db,"\n")  
    createDT(subset(results[[db]], Adjusted.P.value<=0.05), paste("Enrichr Results:",db,"Module", mod))  %>% print()
    cat('\n')
  } 
  cat('\n')
}
