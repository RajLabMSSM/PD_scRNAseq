...
gp1 <- GenePlot(object = pbmc, gene1 = "nUMI", gene2 = "percent.mito", pch.use=20, 
         do.hover=ifelse(interactive==T, T, F), data.hover = "mut")
```
...
```{r PCAplot} 
PCAPlot(object = pbmc, dim.1 = 1, dim.2 = 2, do.hover=ifelse(interactive==T, T, F), data.hover="mut")
```
...
```{r UMAP} 
pbmc <- RunUMAP(object = pbmc, dims.use = 1:10, num_threads=0)
# Plot results
...
tp <- TSNEPlot(object = pbmc, do.label=T, label.size = labSize, do.return=T) 
...
# every cell name
p <- DoHeatmap(object = pbmc, genes.use = top5$gene, slim.col.label=T, remove.key=T) 
...
p <- ggplot(data = avgMarker, aes(x=Gene, y=dx, fill=meanExp)) %>% + geom_tile() %>% + scale_fill_distiller(palette="viridis") 
ifelse(interactive, p %>% ggplotly() %>% toWebGL() %>% print(), print(p))
```
...
if(interactive==T){
    subplot(tp1, tp2, titleX = T, titleY = T ) 
}
```
...
p <- TSNEPlot(object=pbmc, do.label=T, pt.size=0.5, do.return=T) 
ifelse(interactive, p %>% ggplotly() %>% toWebGL() %>% print(), print(p)) 
```
