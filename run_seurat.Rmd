...
subsetBiotypes(DAT, subsetGenes)
```
...
identify_cellTypes_by_avgExpression <- function(DAT, markerDF){
...
DAT <- identify_cellTypes_by_avgExpression(DAT, markerDF)
 
# Get proportions of cell types in each cluster
cluster_proportions <- DAT@meta.data %>% group_by(CellType_AvgExp, post_clustering) %>% 
  tally() %>%
...
if (T==F){
  ...
  ha = HeatmapAnnotation(df = markerDF[c("dx","mut","ID","CellType_DGE","CellType_AvgExp","post_clustering")], which = "row") 
...
DEG_df <-runDGE(DAT, "CellType_AvgExp", "CD14++/CD16+", "CD14++/CD16--") 
```
...
DGE_within_clusters(DAT, "CellType_AvgExp", "CD14++/CD16+", "CD14++/CD16--")
```
...
```{r Enrichr on Clusters} 
for (clust in unique(DAT.markers.sig$cluster)){
  cat("\n")
  cat("### Cluster ",clust,"{.tabset .tabset-fade}\n")
  geneList <- subset(DAT.markers.sig, cluster==clust)$gene
  results <- enrichr(genes = geneList, databases = enrichr_dbs )
  for (db in enrichr_dbs){
    cat("\n")
    cat("#### ",db,"\n")  
    print(createDT(subset(results[[db]], Adjusted.P.value<=0.05), paste("Enrichr Results: ",db,"Cluster ", clust)))
    cat("\n")
  } 
  cat("\n")
}
...
for (mod in unique(modules$moduleColors)){
  ...
  geneList <- subset(modules, moduleColors==mod)$symbol
  results <- enrichr(genes = geneList, databases = enrichr_dbs )
  for (db in enrichr_dbs){
    cat("\n")
    cat("#### ",db,"\n")  
    print(createDT(subset(results[[db]], Adjusted.P.value<=0.05), paste("Enrichr Results:",db,"Module", mod)))
    cat("\n")
  } 
  cat("\n")
}
