...
for (clust in within_clusters){
  ...
  createDT_html(head(dge.dx.clust))
  cat("\n")
}
```
...
for (clust in within_clusters){
  ...
  createDT_html(head(dge.mut.clust))
  cat("\n")
}
```
...
```{r Top Markers}
cds_clusts <- cds_DGE[,(pData(cds_DGE)$Cluster==clust)]
marker_test_res = monocle3::top_markers(cds_clusts,
                                        group_cells_by="cluster",  
                                        genes_to_test_per_group = 1000,
                                        cores=nCores)
data.table::fwrite(marker_test_res, "./Results/cluster_markers.csv")
