...
Create a bar chart of cell type % expressed or show log10 Pvalue for the following genes. What I’d like to see if the AD-related genes more expressed in intermediate vs. classical subset. You can plot “pct.1” and “pct.2” for the following genes and p-value. Then do a simple stat to see if they are enriched for intermediate vs. classical.  Thanks! 

Q: Are AD-related genes more concentered in classical, intermediate or non-classical monocyte subset? 
...
ADgenes <- percent_cells_expressing(DAT, ADgenes, clusterList=c(0,1))


ggplot(data=ADgenes, aes(x=Cluster, y=cellsExpressing, fill=Gene)) + geom_col() + 
...
clustDAT <- SubsetData(DAT, subset.name = "post_clustering", accept.value = c(0,1)) 
 
library(GMD)
heatmap.3(DAT)
DoHeatmap(clustDAT, genes.use = ADgenes$Gene, slim.col.label = T, use.scaled = F)
```
...
# write.csv(MonocyteSubtype.markers, file.path(resultsPath, "MonocyteSubtype.markers_wilcox.csv"), quote = F,row.names = T)
```
