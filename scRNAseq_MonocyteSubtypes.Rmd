...
make_fractionDF <- function (MonocyteSubtype.markers_sig, geneList){
  MonocyteSubtype.markers_sig$Gene <- row.names(MonocyteSubtype.markers_sig)
  pct_df <- melt(subset(MonocyteSubtype.markers_sig, Gene%in%geneList), 
                 id.vars = c("Gene","avg_logFC"), measure.vars = c("pct.2", "pct.1"), 
                 variable.name = "Group", value.name = "FractionCells") 
  pct_df$Group <- ifelse(pct_df$Group=="pct.1", "Cluster0", "Cluster1") 
  return(pct_df)
}

cellFractions_plot <- function (MonocyteSubtype.markers_sig, geneList, disease="Gene List"){ 
  pct_df <- make_fractionDF(MonocyteSubtype.markers_sig, geneList) 
  # Fraction Cells
  cfp <- ggplot(data=pct_df, aes(x=Gene, y=FractionCells, fill=Group)) + geom_col(position="dodge") + 
    labs(title = paste("Fraction of Cells Expressing ",disease,"-associated Genes", sep=""), 
         y="Fraction of Cells", x="Gene") +  
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) 
  print(cfp)
}

logFC_plot <- function(MonocyteSubtype.markers_sig, geneList, disease="Gene List"){
  pct_df <- make_fractionDF(MonocyteSubtype.markers_sig, geneList) 
  # LogFC
  lfcp <- ggplot(data=pct_df, aes(x=Gene, y=avg_logFC, fill=Gene)) + geom_col(position="dodge") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "none") +
     labs(title = paste("logFC of ",disease,"-associated DEGs:\nCanonical vs. Intermediate Monocytes",sep=""), 
          y="avg_logFC", x="Gene") 
  print(lfcp)
} 
...
overlappingGenes_PD <- report_overlap(clustDAT, 
                                      list1=row.names(MonocyteSubtype.markers_sig), 
                                      list2=PDgenes$Gene)
if(length(overlappingGenes_PD)>0){
  FeaturePlot(DAT, features.plot = overlappingGenes_PD, dark.theme = T)
  cellFractions_plot(MonocyteSubtype.markers_sig, overlappingGenes_PD, disease="PD") 
  logFC_plot(MonocyteSubtype.markers_sig, overlappingGenes_PD, disease="PD")

  if(length(overlappingGenes_PD)>1){
    overlap_heatmap(clustDAT, geneList =overlappingGenes_PD, title="Overlapping Genes:\nPD Gene List vs. DGE Genes" )
  } 
} else {cat("There were no overlapping genes between the DGE list and the provided gene list.")} 
```

### AD Genes vs. DGE Overlap

```{r AD Genes vs. DGE Overlap}
overlappingGenes_AD <- report_overlap(clustDAT,
                                      list1=row.names(MonocyteSubtype.markers_sig),
                                      list2=ADgenes$Gene)
if(length(overlappingGenes_AD)>0){
  FeaturePlot(DAT, features.plot = overlappingGenes_AD, dark.theme = T, nCol = 5, 
            cols.use =  c("magenta","grey","green"))
  cellFractions_plot(MonocyteSubtype.markers_sig, overlappingGenes_AD, disease="AD") 
  logFC_plot(MonocyteSubtype.markers_sig, overlappingGenes_AD, disease="AD") 
  
  if(length(overlappingGenes_AD)>1){
    overlap_heatmap(clustDAT, geneList = overlappingGenes_AD, title = "Overlapping Genes:\nAD Gene List vs. DGE Genes" )
  } 
} else {cat("There were no overlapping genes between the DGE list and the provided gene list")}  
```
