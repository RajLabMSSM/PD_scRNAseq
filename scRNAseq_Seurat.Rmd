...
# DAT <- readRDS("../processed_seurat_obj.RDS")
...
__WARNING__: This is a very large object, and will cause errors if you try to push it to GitHub.

```{r save, eval=F}
saveRDS(DAT, file = "../processed_seurat_obj.RDS")
```
...
DAT_id <- Seurat::ScaleData(object = DAT,
                            vars.to.regress = c("nUMI", "percent.mito","ID"))
DEG_dx.id <- runDGE(DAT_id, meta_var = "dx", group1 = "PD", group2 = "control", 
...
# DAT <- readRDS("../processed_seurat_obj.RDS") 
id_key <- DAT$ID
exp_raw<- data.table::data.table(ID=DAT$ID,
                              dx=DAT$dx,
                              mut=DAT$mut,
                              clust=DAT$post_clustering,
                              t(DAT@assays$RNA@scale.data))
exp_median <- exp_raw %>% 
  dplyr::select(-ID) %>%
  dplyr::group_by(dx, clust, mut) %>% 
  dplyr::summarise_all(.funs = median)

medianDGE.test <- function(exp_median, 
                           cluster="all", 
                           metavar="dx", 
                           nThreads=4){
  # pbmcapply::pbmclapply
  res_DT <- parallel::mclapply(colnames(exp_median)[-c(1:3)], function(gene){
    print(gene)
    if(cluster!="all"){
      dat <- subset(exp_median, clust==cluster)
    } else {dat <- exp_median}
    res_dt <- NULL
    res <- NULL
    try({
        res <- stats::wilcox.test(formula(paste(gene,"~",metavar)), 
                                  data=dat)
        res_dt <- data.table::data.table(metavar=metavar,
                                         cluster=cluster,
                                         gene=gene,
    ...
    return(res_dt) 
  }, mc.cores = nThreads) %>% data.table::rbindlist(fill=T)
  res_DT$FDR <- p.adjust(p = res_DT$p.value, method = "fdr")
  return(res_DT)
}
 
  medianDGE.within_clusters <- function(exp_median,
                                        metavar="dx", 
                                        return_data=T){
    res_data <- lapply(unique(exp_median$clust), function(cluster){
     print(paste("Cluster",cluster))
      clust_dge <- medianDGE.test(exp_median=exp_median,
                                  cluster=cluster,
                                  metavar=metavar,
                                  nThreads=4)
      res_path <- paste0("./Results/Median/dge_",metavar,"_cluster",cluster,".tsv")
      data.table::fwrite(clust_dge, res_path)
      if(return_data){
        clust_dge
...
```{r}
# Across Clusters 
dx_dge <- medianDGE.test(exp_median = exp_median, 
                         metavar = "dx", nThreads = 4)  
data.table::fwrite(dx_dge, "./Results/Median/dge_dx_allCluster.tsv")

mut_dge <- medianDGE.test(exp_median = subset(exp_median, mut %in% c("PD","LRRK2")),
                          metavar = "mut") 
data.table::fwrite(mut_dge, "./Results/Median/dge_mut_allCluster.tsv")

  
# Within clusters
withinClust_dx <- medianDGE.within_clusters(exp_median, metavar="dx")
subset(withinClust_dx, FDR<0.05)

withinClust_mut <- medianDGE.within_clusters(subset(exp_median, mut %in% c("PD","LRRK2")), metavar="mut")
subset(withinClust_mut, FDR<0.05)
```
