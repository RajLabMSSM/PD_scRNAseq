...
runDGE <- function(DAT, 
  ...
  if(save_path!=F){
    res_path <- file.path(save_path,
                          paste0("DGE.results_",meta_var,"_cluster",cluster,".tsv.gz"))
...
DGE_within_clusters <- function(DAT, meta_var, group1, group2){ 
  for (clust in unique(DAT@meta.data$post_clustering)){
   ...
   DEG_df <-runDGE(DAT_clustSub, 
                   ...
                   show_plot = F,
                   show_table = F) 
   vol <- volcanoPlot(DEG_df = DEG_df, 
                      caption = paste("Cluster",clust,"DEGs:",group1,"vs.",group2),
                      show_plot = F)
   print(vol)
   createDT_html(DEG_df)
   cat('  \n')  
  } 
}
```
...
```{r Within Clusters - Between Disease Groups, results='asis', warning=F}   
